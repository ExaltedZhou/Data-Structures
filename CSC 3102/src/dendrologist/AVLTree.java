...
/*===> BEGIN: Augmented public methods <===*/   
   ...
   public void levelOrder(Node node,Function func) {
	   if(node == null)
		   return;
	   else { 
		   func.apply(node.data);
		   levelOrder(node.left,func);
		   levelOrder(node.right,func);	   
	   }	   
   }
...
/* BEGIN: Augmented Private Auxiliary Methods */
   ...
   private boolean isFull(Node node)
   {
    ...
    private boolean isComplete(Node node, int index)
    {
    	...
    	int n = numNode(node);
    	if(index>=n||node.bal==BalancedFactor.RH)
    		return false;
    	else
    		return isComplete(node.left,2*index+1)&&isComplete(node.right,2*index+2);
    }  
    
    public int numNode(Node inNode) {
    	if(inNode==null)
    		return 0;
    	else 
    		return numNode(inNode.left)+numNode(inNode.right)+1;
    }
