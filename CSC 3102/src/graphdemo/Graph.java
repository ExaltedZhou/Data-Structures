...
public class Graph<E extends Comparable<E>> implements GraphAPI<E>
{
   ...
   */
   ...
   public boolean isEdge(E fromKey, E toKey)
   {
	  ...
	  Vertex tempFrom = first;
	  while(cmp.compare(fromKey,tempFrom.data)>0) 
		  tempFrom = tempFrom.pNextVertex;
	  Vertex tempTo = first;
	  while(cmp.compare(toKey, tempTo.data)>0)
		  tempTo=tempTo.pNextVertex;
	  Edge tempEdge = tempFrom.pEdge;
	  while(tempEdge!=null&&tempEdge.destination!=tempTo)
		  tempEdge = tempEdge.pNextEdge;
      return  tempEdge!=null;
   }
   ...
   public boolean isPath(E fromKey, E toKey)
   {
	 ...
	 Queue<Vertex> q = new LinkedList();
      if(!isVertex(fromKey)||!isVertex(toKey))
    	  return false;      
      Vertex tempFrom = first;
	  while(tempFrom!=null&&cmp.compare(fromKey,tempFrom.data)!=0) 
		  tempFrom = tempFrom.pNextVertex;
	  Vertex tempTo = first;
	  while(tempTo!=null&&cmp.compare(toKey, tempTo.data)!=0)
		  tempTo=tempTo.pNextVertex;
	  Vertex walkPtr = first;
	  while(walkPtr!=null) {
		  walkPtr.processed = 0;
	  ...
	  q.add(tempFrom);
	  tempFrom.processed=1;
	  while(!q.isEmpty()) {		  
		  Vertex tempF = q.remove();
		  Edge tempE=tempF.pEdge;
		  while(tempE!=null&&tempE.destination!=tempTo){	
			  if(tempE.destination.processed==0) {			 
			  q.add(tempE.destination);
			  tempE.destination.processed=1;		
			  }	
			  tempE=tempE.pNextEdge;
		  }	
		  if(tempE!=null)
			  return true;
	  }
